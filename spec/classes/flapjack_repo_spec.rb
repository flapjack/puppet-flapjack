...
describe 'flapjack::repo', :type => :class do
...
#    end#opt_strings
  ...
  end#input validation
  {
    'debian' => 'apt',
    'RedHat' => 'yum',
  }.each_pair do |os,repo_class|
    context "When on a #{os} host" do
      let (:facts) {{'osfamily' => os, 'lsbdistcodename' => os, 'lsbdistid' => os }}
      context 'when enable_repo is true' do
        let (:params) {{'enable_repo' => true}}
        it "should include the flapjack::repo::#{repo_class} class" do
          should contain_class("flapjack::repo::#{repo_class}")
        end
      end
      context 'when enable_repo is false' do
        let (:params) {{'enable_repo' => false}}
        it "should not include the flapjack::repo::#{repo_class} class" do
          should_not contain_class("flapjack::repo::#{repo_class}")
        end
      end
    end
  end
